...
If a load-bearing claim is *recalled* and concerns external behavior — verify it against the artifact or primary docs **before** stating it as fact. Memory is not a source.
...
- Skipped a layer (data → bloc → UI; or a migration boundary).
...
- `code-review`, security review and any review flow — domain-specific checklists. This skill is the meta-stance applied while running them: ≥2 hypotheses, evidence vs inference, pre-mortem, no sycophancy.
- `simplicity`, `design-principles` — content rules for the answer. This skill governs *how* the answer is reasoned out.
