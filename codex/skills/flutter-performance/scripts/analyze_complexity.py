...
MEMBERSHIP_RE = re.compile(
    r"(\.includes\s*\(|\.indexOf\s*\(|\.find\s*\(|\.findIndex\s*\(|\bin_array\s*\(|"
    r"\bcontains\s*\(|\.where\s*\(|\.firstWhere\s*\(|\.singleWhere\s*\(|"
    r"\.lastWhere\s*\(|\.indexWhere\s*\(|\.any\s*\()"
)
...
DART_BUILD_RE = re.compile(r"\bWidget\s+build\s*\(")
DART_QUERY_RE = re.compile(
    r"(\bselect\s*\(|\.watch\w*\s*\(|\.getSingle\w*\s*\(|\bcustomSelect\s*\(|"
    r"\bcustomStatement\s*\(|\.get\w*\s*\(|\.find\w*\s*\(|\.fetch\w*\s*\(|"
    r"\.load\w*\s*\(|\.query\w*\s*\(|\.read\s*\(|\.post\s*\(|\.put\s*\(|"
    r"\.delete\s*\(|\.patch\s*\()"
)
...
def scan_text(path: Path, root: Path, text: str) -> list[Finding]:
    ...
    if path.suffix in {".jsx", ".tsx", ".js", ".ts"}:
        render_ranges = component_ranges(lines)
    elif is_dart:
        render_ranges = dart_build_ranges(lines)
    else:
        render_ranges = set()
    query_re = DART_QUERY_RE if is_dart else QUERY_IN_LOOP_RE
    render_tokens = (
        [".map(", ".where(", ".sort(", ".fold(", ".reduce("]
        if is_dart
        else [".filter(", ".map(", ".sort(", ".reduce("]
    )
    ...
    for idx, line in enumerate(lines, start=1):
        ...
        if loop_stack and query_re.search(stripped):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
        ...
        if idx in render_ranges and any(token in stripped for token in render_tokens):
            findings.append(
                Finding(
                    rel(path, root),
                    idx,
...
def dart_build_ranges(lines: list[str]) -> set[int]:
    interesting: set[int] = set()
    active = False
    active_until = 0
    brace_balance = 0
    for idx, line in enumerate(lines, start=1):
        if DART_BUILD_RE.search(line):
            active = True
            active_until = idx + 120
            brace_balance = 0
        if active:
