...
Pair with `critical-self-review` (reactive audit triggered by user doubt). This skill prevents the need for it.
...
If a load-bearing claim is *recalled* and concerns external behavior (API, CLI, library, version, system semantics) — verify it (read the file/source, fetch primary docs) **before** stating it as fact. Memory is not a source.
...
- `critical-self-review` — reactive audit after the fact. This skill is preventative.
- `dart-code-review` and any review/security-review flow — domain-specific checklists. This skill is the meta-stance applied while running them: ≥2 hypotheses, evidence vs inference, pre-mortem, no sycophancy.
